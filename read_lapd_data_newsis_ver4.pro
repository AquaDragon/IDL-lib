...
;
;daqconfig 3: no motion list with extra variable scan, set nsteps=1 if no variable scan. 
;             data recorded in following sequence;
;             data 2D (time,shot,chan,step) from daq -> nshots on all channels ->
;             extra variable steps 
;             nsteps and nshots have to provided at the input while using this config.
;
...
;nsteps      :integer, must specify nsteps for daqconfig 1 & 2 & 3. Nsteps is the number of
;               steps in the extra variable specified above. Optional for daqconfig 0
;               If not set, default nsteps = 1 is used.
...
;          Array(nt,nx,ny,nshots,nchan,nsteps) for daqconfig 1 & 2,
;          Array(nt,nshots,nchan,nsteps) for daqconfig 3,
...
PRO read_lapd_data_newsis_ver4, readfname, dataset, t, x, y, z, nt, nx, ny, nz, nchan $
    , theta=theta, phi=phi, sis=sis, trange=trange, xrange=xrange, yrange=yrange, zrange=zrange $
    , daqconfig=daqconfig, shotrange=shotrange, readchan=readchan, nshots=nshots, nsteps=nsteps $
    ...
    IF NOT KEYWORD_SET(nsteps)       THEN nsteps = 1
...
;;;; SPECIFY DEFAULT VARIABLE VALUES --------------------------------------------------------------
    ...
    mem_req=long64(nt)*nx*ny*nz*nshots*nchan*nsteps*4.0e-9 
    ...
    CASE daqconfig OF
        ...
        1:      dataset=fltarr(nt,nx,ny,nshots,nchan,nsteps) ;<< extra steps
        2:      dataset=fltarr(nt,nx,ny,nshots,nchan,nsteps)
        3:      dataset=fltarr(nt,nshots,nchan,nsteps)       ;<< no xy motion, no extra steps
        ...
        ENDIF ELSE BEGIN
           IF nsteps GT 1 THEN BEGIN
               outprint='(Step/Shot/XX/YY/ZZ) = ('+strtrim(step+1,1)+'/' $
                   ...
                   + strtrim(nsteps,1)+'/'+ strtrim(nshots,1)+'/'+strtrim(nx,1)+'/' $
